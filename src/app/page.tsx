...
import { 
  ...
  CheckCircle2, Flame, RefreshCw, Clock, Send, X, Lock, Laptop
} from "lucide-react";
...
export default function Home() {
  ...
  const filteredArticles = selectedCategory 
    ? ALL_ARTICLES.filter(art => art.category.slug === selectedCategory)
    : ALL_ARTICLES;
      ...
      <section id="content-hub" className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 scroll-mt-24">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 border-b border-slate-800 pb-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              Todos ({ALL_ARTICLES.length})
            </button>
            {categoriesList.map((cat) => {
              const count = ALL_ARTICLES.filter(a => a.category.slug === cat.slug).length;
              return (
                <button
                  key={cat.slug}
